...
export default function CarDetailPage(){
  ...
  useEffect(function(){if(CAR.imgs.length<=1)return;var t=setInterval(function(){setIdx(function(c){return(c+1)%CAR.imgs.length})},5000);return function(){clearInterval(t)}},[]);
  ...
  return(
    <div style={{width:"100%",minHeight:"100vh",background:C.bg,...sf(15),color:C.s1,overflowX:"hidden",maxWidth:"100vw"}}>
      <SEOHead title={CAR.name+" — Exotic Car Rental Miami | "+CAR.brand+" | Alfred"} description={"Rent the "+CAR.name+" in Miami. "+CAR.hp+" hp supercar. Full insurance included. Delivered to your door."} path={"/catalog/exotic-cars/"+params.slug} />
      <style>{`
...
@media(max-width:900px){
  ...
  .rev-row{gap:10px!important}
      ...
      <section className="cd-hero" style={{height:"70vh",maxHeight:700,minHeight:450,position:"relative",overflow:"hidden"}}>
        ...
        {CAR.imgs.length>1&&<div onClick={function(){setIdx(function(c){return(c+1)%CAR.imgs.length})}} style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",width:44,height:44,borderRadius:"50%",background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",zIndex:10,backdropFilter:"blur(8px)",transition:"background 0.3s"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.15)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(0,0,0,0.5)"}}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round"><path d="M9 18l6-6-6-6"/></svg>
        </div>}
        {CAR.imgs.length>1&&<div style={{position:"absolute",top:64,right:16,zIndex:10,...sf(11,500),color:"rgba(255,255,255,0.6)",padding:"4px 10px",borderRadius:8,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)"}}>{idx+1} / {CAR.imgs.length}</div>}
      ...
      <div className="page-wrap" style={{marginTop:-40,position:"relative",zIndex:10,opacity:loaded?1:0,transform:loaded?"translateY(0)":"translateY(12px)",transition:"all 0.9s cubic-bezier(0.16,1,0.3,1)"}}>
        <div className="two-col">
          ...
          <div className="right-col">
              ...
              <div style={{padding:"24px 22px"}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:16}}>
                  <div style={{flex:1}}>
                    ...
                    <DarkDatePicker value={pickup} onChange={pickPickup} label="Pickup"/>
                  </div>
            ...
            <div onClick={function(){window.open("https://wa.me/"+WA_NUM+"?text="+encodeURIComponent("Hi Alfred, I'm interested in renting the "+CAR.name+". Could you let me know about availability and pricing?"),"_blank")}} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"13px 0",borderRadius:14,border:"1px solid "+C.bd,marginTop:10,cursor:"pointer",textDecoration:"none",...sf(12,500),color:C.s4,transition:"all 0.3s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor=C.s7;e.currentTarget.style.color=C.s1}} onMouseLeave={function(e){e.currentTarget.style.borderColor=C.bd;e.currentTarget.style.color=C.s4}}>
      ...
      <div ref={specsRef} className="page-wrap" style={{paddingTop:60,marginBottom:40}}>
        {secDiv}
        <p style={{...sf(10,500),color:C.s7,letterSpacing:5,textTransform:"uppercase",marginBottom:20,marginTop:32}}>Performance</p>
      ...
      <div ref={inclRef} className="page-wrap" style={{paddingTop:60,marginBottom:40}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px clamp(14px,3vw,24px)",borderRadius:20,background:"rgba(244,244,245,0.03)",border:"1px solid "+C.bd,marginTop:14}}>
      ...
      <div ref={priceRef} className="page-wrap" style={{paddingTop:60,marginBottom:40}}>
        ...
        <div onClick={function(){window.open("https://wa.me/"+WA_NUM+"?text="+encodeURIComponent("Hi Alfred, I'm interested in renting the "+CAR.name+". Could you let me know about availability and pricing?"),"_blank")}} style={{display:"flex",alignItems:"center",gap:14,padding:"16px clamp(14px,3vw,22px)",borderRadius:20,background:"rgba(244,244,245,0.03)",border:"1px solid "+C.bd,cursor:"pointer",marginTop:14,textDecoration:"none",transition:"border-color 0.3s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor=C.s7}} onMouseLeave={function(e){e.currentTarget.style.borderColor=C.bd}}>

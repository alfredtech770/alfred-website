...
@media(max-width:390px){.n-hero{height:280px!important}.n-title{font-size:28px!important}.search-bar{grid-template-columns:1fr!important}}
      ...
      <div ref={gridRef} className="n-grid" style={{paddingBottom:80}}>
        ...
        ):filtered.map(function(v,i){return <VenueCard key={v.slug} v={v} i={i} vis={gridVis} inProposal={proposal.isInProposal("nightlife",v.slug)} onProposal={function(vv){proposal.addItem({category:"nightlife",id:vv.slug||vv.id,name:vv.name,image:vv.img||vv.hero_image_url,subtitle:(vv.type||"")+" · "+(vv.loc||vv.city||"Miami"),price:null,priceLabel:null,details:(vv.vibe||"")+(vv.music?" · "+vv.music:"")})}}/>})}
      </div>

    ...
    <div ref={ref} style={{position:"relative",flexShrink:0,WebkitTapHighlightColor:"transparent"}} data-filter-drop="true">
      ...
      {open&&<div style={dropStyle}>
        {p.options.map(function(opt){
          ...
          return <div key={opt} onTouchEnd={function(e){e.preventDefault();e.stopPropagation();p.onChange(opt);setOpen(false)}} onClick={function(e){e.stopPropagation();if(isMobile)return;p.onChange(opt);setOpen(false)}} style={{padding:"13px 16px",cursor:"pointer",background:active?"rgba(244,244,245,0.04)":"transparent",borderBottom:"1px solid rgba(44,44,49,0.5)",display:"flex",alignItems:"center",gap:8,...sf(13,active?600:400),color:active?C.s1:C.s4,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",userSelect:"none"}} onPointerEnter={function(e){if(e.pointerType==="mouse")e.currentTarget.style.background="rgba(244,244,245,0.06)"}} onPointerLeave={function(e){if(e.pointerType==="mouse")e.currentTarget.style.background=active?"rgba(244,244,245,0.04)":"transparent"}}>
  ...
  return(<div style={{backgroundColor:C.bg,color:C.s1,minHeight:"100vh"}}>
    ...
    {generating&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,backgroundColor:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
      <div style={{backgroundColor:C.el,padding:32,borderRadius:12,textAlign:"center"}}>
        <div style={{width:40,height:40,margin:"0 auto 16px",animation:"loaderPulse 1.6s ease-in-out infinite"}}><svg width="40" height="40" viewBox="0 0 100 100" fill="none" style={{display:"block"}}><text x="50" y="80" textAnchor="middle" fontFamily="'Times New Roman','Didot','Bodoni 72',Georgia,serif" fontSize="92" fontStyle="italic" fontWeight="500" fill={C.s1}>A</text></svg></div>
      ...
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}@keyframes loaderPulse{0%,100%{opacity:0.45;transform:scale(0.92)}50%{opacity:1;transform:scale(1.04)}}`}</style>
    </div>}
  </div>);
}

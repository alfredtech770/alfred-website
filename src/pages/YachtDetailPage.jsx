...
var TIERS=[{label:"4 hours",hours:4,field:"price_4hr"},{label:"6 hours",hours:6,field:"price_6hr"},{label:"8 hours",hours:8,field:"price_8hr"}];
...
export default function YachtDetailPage(){
  ...
  var YACHT = {
    ...
    brand: yacht.brand,
    body: "Yacht",
    pricePerDay: yacht.price_4hr,
    imgs: yacht.imgs || [yacht.img],
    hp: (yacht.size_ft||"")+"ft",
    torque: "",
    engine: yacht.brand,
    transmission: "Charter",
    drive: yacht.city,
    topSpeed: yacht.location,
    acceleration: (yacht.max_passengers||"")+" guests",
    weight: "",
    seats: yacht.max_passengers,
    rating: 5.0,
    ...
    deposit: yacht.security_deposit || 1000,
  ...
  useEffect(function(){var t=setInterval(function(){setIdx(function(c){return(c+1)%YACHT.imgs.length})},5000);return function(){clearInterval(t)}},[]);
  ...
  .rev-row{gap:10px!important}
      ...
      <section className="cd-hero" style={{height:"70vh",maxHeight:700,minHeight:450,position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,transform:"translateY("+heroY+"px) scale("+heroScale+")"}}>
          {YACHT.imgs.map(function(img,i){return <img key={i} src={img} alt="" onClick={function(){setLightbox(true)}} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 60%",opacity:i===idx?1:0,transition:"opacity 0.8s ease",filter:"brightness(1.2)",cursor:"pointer"}}/>})}
        </div>
        ...
        <div onClick={function(){setIdx(function(c){return(c+1)%YACHT.imgs.length})}} style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",width:44,height:44,borderRadius:"50%",background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",zIndex:10,backdropFilter:"blur(8px)",transition:"background 0.3s"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.15)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(0,0,0,0.5)"}}>
        ...
        <div style={{position:"absolute",top:64,right:16,zIndex:10,...sf(11,500),color:"rgba(255,255,255,0.6)",padding:"4px 10px",borderRadius:8,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)"}}>{idx+1} / {YACHT.imgs.length}</div>
        ...
        <div style={{position:"absolute",top:64,left:16,display:"flex",gap:6,zIndex:10,flexWrap:"wrap"}}>
          ...
          <span style={{...sf(9,500),color:C.s5,padding:"4px 8px",borderRadius:8,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(12px)"}}>{YACHT.drive}</span>
      ...
      <div className="page-wrap" style={{marginTop:-40,position:"relative",zIndex:10,opacity:loaded?1:0,transform:loaded?"translateY(0)":"translateY(12px)",transition:"all 0.9s cubic-bezier(0.16,1,0.3,1)"}}>
        <div className="two-col">
          ...
          <div className="left-col">
            ...
            <div style={{marginBottom:40}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14,flexWrap:"wrap"}}>
                <div style={{display:"flex",alignItems:"center",gap:5}}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={C.s6} strokeWidth="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg><span style={{...sf(12),color:C.s5}}>{YACHT.location}</span></div>
                <div style={{width:1,height:12,background:C.bd}}/>
                <div style={{display:"flex",alignItems:"center",gap:4}}><svg width="11" height="11" viewBox="0 0 24 24" fill={C.gold} stroke={C.gold} strokeWidth="1"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg><span style={{...sf(13,600),color:C.s1}}>{YACHT.rating}</span><span style={{...sf(11),color:C.s6}}>({YACHT.reviews})</span></div>
                {YACHT.color&&<><div style={{width:1,height:12,background:C.bd}}/><span style={{...sf(12),color:C.s5}}>{YACHT.color}</span></>}
              </div>
              <div style={{display:"flex",alignItems:"baseline",gap:6}}>
                <span style={{...sf(34,700),color:C.s1}}>${(YACHT.pricePerDay||0).toLocaleString()}</span>
                <span style={{...sf(14),color:C.s6}}>/4hr</span>
              </div>
            </div>
            ...
            <div ref={specsRef} style={{paddingTop:20,marginBottom:24}}>
              ...
              <div className="spec-grid" style={{marginBottom:14,opacity:1,transition:"all 0.9s ease 0.1s"}}>
                {[{emoji:"⚡",value:""+YACHT.hp,unit:"",label:"Size"},{emoji:"⏱",value:YACHT.acceleration,unit:"",label:"Capacity"},{emoji:"🏁",value:YACHT.topSpeed,unit:"",label:"Location"}].map(function(s,i){
              ...
              <div className="detail-grid" style={{opacity:1,transition:"all 0.9s ease 0.2s"}}>
                {[{l:"Brand",v:YACHT.engine},{l:"Type",v:YACHT.transmission},{l:"City",v:YACHT.drive},{l:"Guests",v:""+YACHT.seats},{l:"Vessel",v:YACHT.body},{l:"Location",v:YACHT.location}].filter(function(d){return d.v}).map(function(d,i){
            ...
            <div ref={inclRef} style={{paddingTop:20,marginBottom:24}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px clamp(14px,3vw,24px)",borderRadius:20,background:"rgba(244,244,245,0.03)",border:"1px solid "+C.bd,marginTop:14,opacity:1,transition:"opacity 0.8s ease 0.3s"}}>
                ...
                <div style={{...sf(24,700),color:C.s1}}>${YACHT.deposit.toLocaleString()}</div>
              </div>
            </div>
            ...
            <div ref={priceRef} style={{paddingTop:20,marginBottom:24}}>
              ...
              <div style={{borderRadius:20,background:C.el,border:"1px solid "+C.bd,padding:"18px 20px",opacity:1,transition:"all 0.9s ease 0.15s"}}>
                {TIERS.map(function(t,i){var active=tier.field===t.field;var price=yacht[t.field]||0;return(<div key={i}>{i>0&&<div style={{height:0.5,background:C.bd}}/>}<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"13px 4px"}}><div style={{display:"flex",alignItems:"center",gap:8}}>{active&&<div style={{width:4,height:4,borderRadius:"50%",background:C.s1}}/>}<span style={{...sf(13,active?500:400),color:active?C.s1:C.s6}}>{t.label}</span></div><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{...sf(15,active?600:400),color:active?C.s1:C.s6}}>${price.toLocaleString()}</span><span style={{...sf(10),color:C.s7}}>/charter</span></div></div></div>)})}
              </div>
              <a href={"https://wa.me/message/DAO44K3XCXK3F1?text="+encodeURIComponent("Hi, I have a question about the "+YACHT.name)} target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"center",gap:14,padding:"16px clamp(14px,3vw,22px)",borderRadius:20,background:"rgba(244,244,245,0.03)",border:"1px solid "+C.bd,cursor:"pointer",marginTop:14,textDecoration:"none",transition:"border-color 0.3s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor=C.s7}} onMouseLeave={function(e){e.currentTarget.style.borderColor=C.bd}}>
            ...
            <div ref={revRef} style={{paddingTop:20,marginBottom:24}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:32,marginBottom:20}}>
                ...
                <span style={{...sf(12),color:C.s6,opacity:1}}>{YACHT.reviews} reviews</span>
              </div>
          ...
          <div className="right-col">
            ...
            {/* WhatsApp quick */}
            <a href={"https://wa.me/message/DAO44K3XCXK3F1?text="+encodeURIComponent("Hi, I have a question about the "+YACHT.name)} target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"13px 0",borderRadius:14,border:"1px solid "+C.bd,marginTop:10,cursor:"pointer",textDecoration:"none",...sf(12,500),color:C.s4,transition:"all 0.3s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor=C.s7;e.currentTarget.style.color=C.s1}} onMouseLeave={function(e){e.currentTarget.style.borderColor=C.bd;e.currentTarget.style.color=C.s4}}>
      ...
      <section ref={ctaRef} style={{padding:"120px 0 100px",position:"relative"}}>
        ...
        <div style={{textAlign:"center",maxWidth:500,margin:"0 auto",padding:"0 40px"}}>
          ...
          <h2 style={{...sf(44,600),letterSpacing:-1.5,lineHeight:1.1,opacity:1,transition:"all 0.9s ease 0.15s"}}>Ready to charter<br/>the {YACHT.name}?</h2>
          <p style={{...sf(15,400),color:C.s5,lineHeight:1.7,marginTop:16,marginBottom:36,opacity:1,transition:"opacity 0.8s ease 0.3s"}}>Tell Alfred your dates and we'll arrange everything. Captain, crew, and open water.</p>
          ...
          <p style={{...sf(12),color:C.s6,marginTop:20,opacity:1,transition:"opacity 0.8s ease 0.6s"}}>Captain included · Fuel included · 24/7 concierge</p>
        </div>
      </section>

...
//  Alfred App
//
//  Yachts browse view — grid of yacht cards with filters.
//

import SwiftUI

struct YachtsView: View {
    @Binding var selectedCity: String
    @Binding var isFilterVisible: Bool

    @State private var venues: [Yacht] = []
    @State private var isLoading = true
    private let service = YachtService()
                ...
                LazyVStack(spacing: 16) {
                    ...
                    if isLoading {
                        ...
                        VStack(spacing: 12) {
                            ...
                            Text("No yachts found")
                                .font(.system(size: 14))
                                .foregroundStyle(A.s5)
                        }.padding(.top, 60)
                    } else {
                        ForEach(venues) { y in
                            NavigationLink(value: y) {
                                AlfredLandscapeCard(yacht: y)
                            }
                            .buttonStyle(.plain)
                        }
                        .padding(.horizontal, 20)
    ...
    private func loadVenues() async {
        isLoading = true
        do { venues = try await service.getYachts(city: selectedCity) } catch { venues = [] }
        isLoading = false
    }
}

...
//  Alfred App
//
//  Accommodation detail view (pushed via NavigationStack).
//
...
struct AccommodationDetailView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 0) {
                    if let url = accommodation.heroImageURL, let imageURL = URL(string: url) {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        if let type = accommodation.accommodationType {
                            Text(type)
                                .font(.system(size: 14))
                                .foregroundStyle(A.s5)
                        }
                        if let city = accommodation.city {
                            Text(city)
                                .font(.system(size: 14))
